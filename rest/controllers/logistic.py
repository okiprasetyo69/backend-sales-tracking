...
class LogisticController(object):
    ...
    def import_packing_slip(self, file, user_id: 'int'):
        ...
        for idx in df_parent_json:
            invno = idx
            docno = df['DOCUMENTNUMBER'][0]
            if isinstance(docno, numpy.int64):
                invno = numpy.int64(invno)
            df_product = df[['ITEMKEY', 'ITEMDESCRIPTION', 'QTYSHIPPED']].loc[df['DOCUMENTNUMBER'] == invno]
        ...
        if search:
            if dropdown:
                where += """AND (ps.sales_order_code LIKE '%{0}%' OR ps.code LIKE '%{0}%' OR b.name LIKE '%{0}%' 
                        OR e.name LIKE '%{0}%' OR c.name LIKE '%{0}%' OR ps.customer_code LIKE '%{0}%')""".format(
                    search)
            else:
                where += """AND (ps.sales_order_code LIKE '%{0}%' OR ps.code LIKE '%{0}%' OR b.name LIKE '%{0}%' 
                        OR e.name LIKE '%{0}%' OR c.name LIKE '%{0}%' OR ps.customer_code LIKE '%{0}%')""".format(
