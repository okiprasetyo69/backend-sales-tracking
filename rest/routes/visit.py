...
from rest.controllers import VisitController, BranchesController, CustomerController, SalesController, \
    ApprovalController, SalesActivityController, CisangkanController
...
def get_all_visit_plan_summary(plan_id):
    ...
    # response['message'] = visit_plan_summary["data"].__len__()
    ...
    output = []
    for i in visit_plan_summary["data"]:
        output.append(cc.loadImageFromPath(i))
        
    visit_plan_summary["data"] = output
    response['error'] = 0
    response['data'] = visit_plan_summary
    return jsonify(response)
